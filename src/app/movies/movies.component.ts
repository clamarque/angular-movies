...
export class MoviesComponent implements OnInit {
    ...
    swipe(currentIndex: number, action = this.SWIPE_ACTION.RIGHT) {
        if (action === this.SWIPE_ACTION.RIGHT || action === this.SWIPE_ACTION.LEFT) this.setPage(this.parameter, currentIndex);
    }
    ...
    setPage(param: any, page: number) {
        ...
        if (typeof param === "number") this.dataService.getGenreMovie(param, this.currentPage).subscribe(response => this.movies = response)
